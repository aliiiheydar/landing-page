...
const addExpert = async (
  ...
  try {
    ...
    const phoneCheckingFilter = {
      websiteName,
      'contactUs.experts.phone': phone
    }
    const existingExpertWithThisPhone = await SiteInfo.findOne(phoneCheckingFilter).exec()
    if(existingExpertWithThisPhone) {
      return {
        success: false,
        error: {
          message: errorMessages.siteInfo.phoneIsTaken,
          statusCode: statusCodes.badRequest
        }
      }
    }
...
const editExpert = async (
  ...
  try {
    ...
    if(updates.phone) {
      ...
      const filter = {
        websiteName,
        'contactUs.experts.phone': updates.phone
      }
      const existingExpert = await SiteInfo.findOne(filter).exec()
      if(existingExpert) {
        return {
